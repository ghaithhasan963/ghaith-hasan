# ghaith-hasan
about me
<h1 align="center">👨‍🏫 Ghaith Hasan | غيث حسن</h1>

<p align="center">
Educator | Developer | Cybersecurity Instructor  
💻 Founder of Digital Shadow Academy | 🇸🇾 Sweida, Syria  
</p>

---

## 🌟 نبذة عني

أنا غيث، مهندس ومعلّم تقني مختص في البرمجة والأمن السيبراني.  
أدرّس في معاهد بسويداء وأعمل حاليًا على بناء منصة تعليمية عربية تدمج التقنية، المهارات الحياتية، والتعلّم التفاعلي بأسلوب متطور يشبه كورسات الجامعات العالمية.

---

## 🧠 مجالات اهتمامي

- ✨ بناء منصات تعليمية تفاعلية
- 🛡️ تطوير كورسات الأمن السيبراني
- 💻 تصميم واجهات تعليمية حديثة
- 🤖 استخدام الذكاء الاصطناعي لإنتاج المحتوى
- 🧭 تخطيط استراتيجي للتوسع والربح في التعليم الرقمي

---

## 📂 مشاريع مميزة

| 💼 المشروع              | الوصف                                         | الحالة      |
|-----------------------|-----------------------------------------------|--------------|
| Digital Shadow Academy | منصة تعليمية عربية بنظام Freemium            | قيد التطوير |
| LMS - Cyber Training   | نظام تدريب خاص بالأمن السيبراني              | تجريبي      |
| Portfolio Site         | موقعي الشخصي لعرض خدماتي ومشاريعي           | منشور       |

🔗 روابط:  
- 🌐 [منصتي الشخصية](https://ghaithhasan.onrender.com)

- 📧 ghaithhasan963@gmail.com  
- 📱 Telegram: @ghaetg1111

---

## ⚙️ المهارات التقنية

```txt
Languages:     HTML · CSS · JavaScript · Python · Bash
Frameworks:    Tailwind · Flask · Frappe · React 
Platforms:     GitHub · Netlify · Firebase · Docker
Tools:         VS Code · Figma · Canva · ChatGPT · Git
Specialties:   Course Planning · Content Strategy · AI-Driven Education

<div align="center"> <img src="https://github-readme-stats.vercel.app/api?username=ghaithhasan963&show_icons=true&theme=tokyonight" /> <br> <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ghaithhasan963&layout=compact&theme=tokyonight" /> </div>

🧭 هدفي القادم
"بناء أكاديمية عربية رقمية تقدم محتوى مجاني ومدفوع، تعليمي ومهاري، ضمن تجربة مستخدم عالية الجودة، تشمل كورسات، مجتمع رقمي، وشهادات إتمام معتمدة."

<h4 align="center">شكراً لزيارتك... المعرفة مشاركة، ونتشاركها معاً ✨</h4>
